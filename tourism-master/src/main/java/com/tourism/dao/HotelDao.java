...
public interface HotelDao extends PagingAndSortingRepository<Hotel, Long>,JpaSpecificationExecutor<Hotel>,JpaRepository<Hotel,Long> {
    ...
    @Modifying
    @Query("UPDATE Hotel SET address=?1, bed=?2, day=?3, detail=?4,  price=?6, title=?7, star=?8 WHERE id = ?9")
    int updateHotelNoImg(String address, String bed, String day, String detail,  String price, String title, String star, String id);
}
